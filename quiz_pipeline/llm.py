...
【题库自洽性】题目和答案必须独立自洽，禁止出现"文中/根据材料/如上图/上文/本节"等指代原文出处的表述。把"文中指出"改成直接陈述结论。读者看不到原文，只看到题目本身。

【避免重复】同一个知识点只出一道最有代表性的题，不要换个问法重复考查同一内容；优先覆盖不同考点而非堆叠数量。

【难度判定标准】严格按以下锚点标注 difficulty：
- 易：单一概念的记忆/复述（是什么、有哪些）。
- 中：多个要点的对比、归纳或分类（区别、各阶段特点、如何对应）。
- 难：机制原理推导、设计权衡、为什么这样设计、优缺点取舍。
偏向如实评估，不要把需要对比归纳的题判为"易"。"""
