# ⓾ LLM实战：从0开始训练Qwen2.5（PT+SFT+DPO）

<!-- source: yuque://zhongxian-iiot9/hlyypb/rgol4a85w4nfp51x -->

## 简介
:::color3
**简介**：mini_qwen是一个从头开始训练的1B参数的大型语言模型(LLM)项目，**<font style="color:#ED740C;">包括预训练(PT)、微调(SFT)和直接偏好优化(DPO)3个部分</font>**。其中预训练和微调仅需要12G显存即可训练，直接偏好优化仅需要14G显存即可训练，这意味着使用T4显卡就可以开始你的训练之旅。

github：[https://huggingface.co/qiufengqijun](https://huggingface.co/qiufengqijun)

huggingface：[https://huggingface.co/qiufengqijun](https://huggingface.co/qiufengqijun)

:::

![](https://cdn.nlark.com/yuque/0/2025/png/29769680/1744789347983-75fcea47-8447-4a9c-8420-7ab9255a60ca.png?x-oss-process=image%2Fformat%2Cwebp)

:::color5
**<font style="color:#601BDE;">1.Mini Qwen介绍</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>

:::

mini_qwen是以**<font style="color:#74B602;">Qwen2.5-0.5B-Instruct</font>**模型为基础，通过扩充模型隐藏状态层数、隐藏状态维度和注意力头数，增加参数量到1B，并进行参数随机初始化。

+ 训练数据使用北京智源人工智能研究院的预训练(16B token)、微调(9M 条)和偏好数据(60K 条)，
+ 使用flash_attention_2进行加速。
+ 使用deepspeed在6张H800上训练25h(pt 1epoch)、43h(sft 3epoch)、1h(dpo 3epoch)。

这是一次非常有趣且有价值的尝试，在整个过程中，本项目探究了**<font style="color:#74B602;">尺度定律(scaling law)、复读机现象与微调阶段的知识注入</font>**，也解决了很多bug。本项目将尽可能详细地介绍整个训练过程。

:::color5
**<font style="color:#601BDE;">2.参考资料</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>

:::

本项目主要参考以下项目和资料：

+ [https://github.com/zhanshijinwat/Steel-LLM](https://github.com/zhanshijinwat/Steel-LLM) [数据来源]
+ [https://www.modelscope.cn/organization/BAAI](https://www.modelscope.cn/organization/BAAI) [数据来源]
+ [https://github.com/jiahe7ay/MINI_LLM](https://github.com/jiahe7ay/MINI_LLM) [代码流程]
+ [https://github.com/hiyouga/LLaMA-Factory](https://github.com/hiyouga/LLaMA-Factory) [代码流程]
+ [https://www.bilibili.com/video/BV1azH1e8Ekf?spm_id_from=333.788.videopod.sections&vd_source=63b3850fa9262e89716266c278491604](https://www.bilibili.com/video/BV1azH1e8Ekf?spm_id_from=333.788.videopod.sections&vd_source=63b3850fa9262e89716266c278491604) [LLaMA-Factory调试debug]
+ [https://huggingface.co/docs](https://huggingface.co/docs) [各种高级api，包括Trainer、TRL(SFTTrainer和DPOTrainer)、Accelerate]
+ [https://www.bilibili.com/video/BV1KX4y1a7Jk/?vd_source=63b3850fa9262e89716266c278491604](https://www.bilibili.com/video/BV1KX4y1a7Jk/?vd_source=63b3850fa9262e89716266c278491604) [Trainer、Accelerate教程]
+ [https://github.com/huggingface/trl/issues](https://github.com/huggingface/trl/issues) [TRL使用过程中各种问题]

## 快速开始
:::color3
**简介**：mini_qwen是一个从头开始训练的1B参数的大型语言模型(LLM)项目，**<font style="color:#ED740C;">包括预训练(PT)、微调(SFT)和直接偏好优化(DPO)3个部分</font>**。其中预训练和微调仅需要12G显存即可训练，直接偏好优化仅需要14G显存即可训练，这意味着使用T4显卡就可以开始你的训练之旅。<font style="color:#D22D8D;"> (by草莓师姐)</font>
...
## 模型下载链接
:::color3
**简介：**本项目将训练过程中的几乎所有模型都进行了保存，下载链接如下：<font style="color:#D22D8D;"> (by草莓师姐)</font>
...
:::color5
**<font style="color:#601BDE;">3.偏好数据</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>
...
## 训练流程
:::color5
**<font style="color:#601BDE;">1.模型与数据下载</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>

:::

若从头训练，模型不需要下载，本项目已经包含模型配置文件。若想基于现有模型进行继续训练，可按需下载模型参数。 数据集包含多种行业多种类别数据，可按需下载。
...
:::color5
**<font style="color:#601BDE;">2.预训练</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>

:::

本项目在预训练阶段，调整模型参数至1B，使用Trainer进行训练，**<font style="color:#74B602;">并参考LLaMA-Factory，对数据进行序列打包(sequences packing)，使用Accelerate和deepspeed进行分布式训练</font>**，使用flash_attention_2进行加速，设置序列长度为1024。  
	**<font style="color:#74B602;">预训练数据格式参考qwen，在文本末尾添加结束符。</font>**
...
:::color5
**<font style="color:#601BDE;">3.微调</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>

:::

本项目在微调阶段，使用TRL中的SFTTrainer进行训练，**<font style="color:#74B602;">使用Accelerate和deepspeed进行分布式训练</font>**，使用flash_attention_2进行加速，设置序列长度为1024，对于多轮对话数据，仅训练第一轮对话数据。  
	TRL并未采用最新版本，而是采用了较低版本(0.11.4)，低版本与高版本的参数设置存在差异，使用不正确的版本运行会报错。  
**注意：请确保 pip install trl==0.11.4**  
	**<font style="color:#74B602;">微调数据格式参考qwen，在文本前后位置添加特殊字符。</font>**
...
:::color5
**<font style="color:#601BDE;">4.直接偏好优化（DPO）</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>
...
****pip install trl==0.11.4**  
**pip install transformers==4.45.0**  
**<font style="color:#74B602;">偏好数据格式参考qwen，在文本前后位置添加特殊字符。</font>**
...
:::color5
**<font style="color:#601BDE;">5.模型评估</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>
...
## 结果分析与模型评估
:::color5
**<font style="color:#601BDE;">1.预训练评估</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>

:::

+ **<font style="color:#117CEE;">结果分析</font>**
...
![](https://github.com/qiufengqijun/mini_qwen/raw/main/images/training_loss_pt.png)![](https://cdn.nlark.com/yuque/0/2025/png/29769680/1749529804049-6210822a-f0b7-4619-93a3-849725582102.png)

+ **<font style="color:#117CEE;">模型评估</font>**
...
![](https://cdn.nlark.com/yuque/0/2025/png/29769680/1749529829212-1e479a48-6659-43da-8da9-83b23e597184.png)
...
![](https://github.com/qiufengqijun/mini_qwen/raw/main/images/training_loss_pt_comparison.png)![](https://cdn.nlark.com/yuque/0/2025/png/29769680/1749529843208-ba8dd146-2c75-4d61-8fa8-9ca645003f40.png)

+ 将上图局部放大(使用utils/plot_losses.py)，可以发现pt1和pt2几乎完全重合，这意味着常规方法和序列打包可能并没有太大差异。 pt3的最终loss小于pt1，这说明了中英文混合数据有助于降低loss。对于尺度定律的探究，具体来说，随着数据量的增加，loss的初始值逐渐增大，但最终的loss值会逐渐减小。

![](https://github.com/qiufengqijun/mini_qwen/raw/main/images/training_loss_pt_comparison_part.png)![](https://cdn.nlark.com/yuque/0/2025/png/29769680/1749529855029-69cde190-a2c0-4bb2-96ee-c2a4a66395af.png)
...
:::color5
**<font style="color:#601BDE;">2.微调评估</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>
...
![](https://cdn.nlark.com/yuque/0/2025/png/29769680/1749529904416-8e6ae7ea-6955-40a0-8e74-bf2c1c16e564.png)
...
:::color5
**<font style="color:#601BDE;">3.DPO评估</font>**<font style="color:#D22D8D;"> (by草莓师姐)</font>
...
![](https://github.com/qiufengqijun/mini_qwen/raw/main/images/training_loss_dpo.png)![](https://cdn.nlark.com/yuque/0/2025/png/29769680/1749529928823-23e75e0e-4874-4c2c-939d-72adf6252fcb.png)
...
按照魔塔教程下载数据 [https://www.modelscope.cn/docs/datasets/download](https://www.modelscope.cn/docs/datasets/download)
...
使用的方式可以参考 [https://huggingface.co/docs/trl/v0.11.4/en/sft_trainer#using-flash-attention-2](https://huggingface.co/docs/trl/v0.11.4/en/sft_trainer#using-flash-attention-2)
...
参考 [https://huggingface.co/docs/trl/v0.11.4/en/sft_trainer#using-tokenids-directly-for-responsetemplate](https://huggingface.co/docs/trl/v0.11.4/en/sft_trainer#using-tokenids-directly-for-responsetemplate)
...
3. flash-attn要对应自己的pytorch、cuda、python版本下载，可以参考 [https://blog.csdn.net/a61022706/article/details/141570792](https://blog.csdn.net/a61022706/article/details/141570792)
4. deepspeed的新版本 有梯度同步的问题 可以设置累积梯度为1 或者参考这个issue对版本降级 [deepspeedai/DeepSpeed#6793](https://github.com/deepspeedai/DeepSpeed/issues/6793)
